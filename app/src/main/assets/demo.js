...
if(!window.Native){
 ...
 function demoEligibility(thread,samples=relationships[thread]?.samples||''){const history=histories[thread]||inbox.filter(x=>x.thread_id===thread);const entries=[...history.filter(m=>[1,2].includes(m.type)).map(m=>({speaker:m.type===2?'me':'them',text:m.body})),...demoTurns(samples)];const unique=new Set(entries.map(m=>m.speaker+':'+m.text.normalize('NFKC').toLowerCase().replace(/\s+/g,' ').trim()));const owner=[...unique].filter(x=>x.startsWith('me:')).length,incoming=unique.size-owner,total=unique.size;return{eligible:total>=20&&owner>=5&&incoming>=5,total,owner,incoming,minimumTotal:20,minimumOwner:5,minimumIncoming:5};}
 ...
 case 'historyLearningStatus':return{phase:'waiting',ready:false};
 case 'retryHistoryLearning':throw new Error('History preparation runs on your paired Pixel.');
 ...
 case 'saveProfile':{
  ...
  const readiness=demoEligibility(p.thread,p.samples);if(p.autoDraft&&!readiness.eligible)throw new Error('Automatic replies need 20 messages, including 5 from each person.');
