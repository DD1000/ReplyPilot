...
let pairingText='',cloudStatus='',ownerViewsText=null; // ownerViewsText: unsaved Settings edits, null when unchanged
...
// The owner's own opinions. Autopilot uses them when someone asks what they think.
function ownerViewsView(){if(!native||typeof state.ownerViews!=='string')return '';const text=ownerViewsText??state.ownerViews,changed=ownerViewsText!==null&&ownerViewsText.trim()!==state.ownerViews;return `<div class="panel owner-views-panel" id="owner-views-panel"><h3>Your views</h3><p>What you think about things people might ask you, like AI, work, sports or movies. When someone asks your opinion, Autopilot answers with these views in your own texting style. Leave it empty and Autopilot keeps its opinions light.</p><label for="owner-views">What you think</label><textarea id="owner-views" maxlength="1200" placeholder="For example: Big on AI, I think it'll take over a lot of repetitive work. Lakers fan. Not into horror movies.">${esc(text)}</textarea><div class="connection-actions"><button class="secondary" data-action="save-owner-views" ${busy||!changed?'disabled':''}>Save views</button><small>${text.length}/1,200</small></div></div>`;}
// Android's battery limits can hold Autopilot's work until the phone wakes. The owner lifts them once.
function batteryView(){if(!native||typeof state.batteryUnrestricted!=='boolean')return '';const ok=state.batteryUnrestricted;return `<div class="setting" id="battery-setting"><div><div class="label">Reply while locked</div><p>${ok?'Autopilot can reply right away while your phone is locked or asleep.':'Android is limiting Reply Pilot’s battery use, so Autopilot replies can wait until you unlock your phone. Allow it once to reply right away.'}</p></div><button class="secondary" data-action="battery" ${ok?'disabled':''}>${ok?'Allowed':'Allow'}</button></div>`;}
function batteryNoteView(edit){if(!native||state.batteryUnrestricted!==false||profileMode(edit)!=='autopilot')return '';return `<div class="battery-note" id="battery-note" role="note"><p><strong>Replies may wait while your phone is locked.</strong> Android is limiting Reply Pilot’s battery use. Allow it once so Autopilot can answer right away.</p><button class="secondary" data-action="battery">Allow</button></div>`;}
...
// "Use Astra for this chat": the owner's server uses its stronger model for this chat's AI replies.
const premiumSaving=new Set();
function premiumView(){const data=replyProfileRecord().data;if(!data||typeof data.premiumReplies!=='boolean')return '';const saving=premiumSaving.has(current?.thread_id);return `<div class="setting premium-setting" id="premium-setting"><div><label class="label" for="premium-replies">Use Astra for this chat</label><p>OpenAI’s smartest model writes this chat’s AI replies. Better with jokes, tone and tricky questions, a little slower, and about 5× the cost: roughly 5–7¢ per reply instead of about 1¢.</p></div><input type="checkbox" id="premium-replies" ${data.premiumReplies?'checked':''} ${saving?'disabled':''}></div>`;}
async function setPremiumReplies(on){
 if(!current||!conversation)return;const thread=current.thread_id,record=replyProfileRecord(thread);if(!record.data||premiumSaving.has(thread))return;
 premiumSaving.add(thread);syncReplyModes();
 try{const result=await api('setPremiumReplies',{thread,expectedAddress:profileContext(thread).address||current.address||'',premium:on});if(record.data)record.data.premiumReplies=result?.premiumReplies===true;if(current?.thread_id===thread)toast(result?.premiumReplies?'Astra will write this chat’s replies.':'This chat is back on the standard model.');}
 catch(error){toast(error.message);}
 finally{premiumSaving.delete(thread);if(current?.thread_id===thread)syncReplyModes();}
}
function replyModesView(edit){
 ...
 return `<div id="reply-modes">${checked?trainPanelView():''}${modes}${checked?premiumView():''}${batteryNoteView(edit)}${status}</div>`;
}
...
root.addEventListener('input',e=>{if(e.target.id==='home-address'){homeForm().address=e.target.value;homeForm().dirty=true;closeHomeCandidates(false);syncLocationUI();return;}if(e.target.id==='location-enabled'){changeLocation('saveLocation',{enabled:e.target.checked});return;}if(e.target.id==='owner-views'){ownerViewsText=e.target.value;const save=root.querySelector('[data-action="save-owner-views"]');if(save)save.disabled=busy||ownerViewsText.trim()===state.ownerViews;const count=root.querySelector('#owner-views-panel small');if(count)count.textContent=`${ownerViewsText.length}/1,200`;return;}if(e.target.id==='pairing-code'){pairingText=e.target.value;root.querySelector('[data-action="connect-cloud"]').disabled=busy||!native||!pairingText.trim();}const testKey={'test-message':'message','test-relationship':'relationship','test-examples':'examples'}[e.target.id];if(testKey){cloudTest[testKey]=e.target.value;cloudTest.error='';document.getElementById('test-error').textContent='';root.querySelector('[data-action="test-run"]').disabled=!canTest();}if(e.target.id==='premium-replies'){setPremiumReplies(e.target.checked);return;}if(e.target.name==='person-mode'){
 const edit=relationshipEdit(),enabled=e.target.value==='autopilot';if(enabled&&!profileEligibility(edit).eligible){toast('Train Autopilot for this chat first.');render();return;}
...
case 'save-owner-views': {busy=true;busyThread=current?.thread_id??null;render();try{const saved=await api('saveOwnerViews',{text:ownerViewsText??state.ownerViews??''});state.ownerViews=saved.ownerViews;ownerViewsText=null;toast('Your views are saved.');}finally{busy=false;render();}break;}
...
case 'permissions':case 'alarms':case 'battery': await api(action);await refresh(true);break;
