...
let busyThread=null;
let historyLearningRequest=null,historyLearningRevision=0,historyLearningNextCheck=0;
...
function clearPrivateChats(){
 invalidateHistoryLearning();resetArchiveReads();state.historyCache=null;historyCacheError='';sendJobUpdates.clear();sendStateRevision++;sendStateRequest=null;
...
function syncLiveComposer(){
 ...
 syncHydrationNotice();syncReplyHoldUI();syncShareReview();syncHistoryLearningStatus();tick();
}
...
function mediaInsightView(message){
 ...
 const item=mediaInsights.get(mediaInsightKey(current.thread_id,message._id)),enabled=historyLearningReady()&&chatReady()&&conversation?.relationship?.cloudEnabled===true&&!conversation.readOnly;
...
async function loadReplyProfile(thread=current?.thread_id){
 if(!thread||thread!==current?.thread_id||previewDenied)return;const record=replyProfileRecord(thread),epoch=threadEpoch,request=++record.request,generation=privateGeneration,address=current.address;record.loading=true;record.error='';syncHistoryLearning();
...
async function recheckReplyHistory(){
 ...
 const samples=edit.samples,request=++record.request;record.loading=true;record.error='';syncHistoryLearning();
 ...
 finally{if(current?.thread_id===thread&&epoch===threadEpoch&&request===record.request){record.loading=false;syncHistoryLearning();}}
}
...
function replyModesView(edit){
 const ready=profileEligibility(edit),record=replyProfileRecord(),checked=profileReady(),checking=record.loading||!checked&&!record.error;
 const options=[['off','Off','Write your own replies.'],['autopilot','Autopilot','Reply automatically using this conversation and your guidance.']];
 const modes=radioGroup('person-mode','AI replies',options,profileMode(edit));
 if(checked&&ready.eligible&&!record.error)return `<div id="reply-modes">${modes}</div>`;
 const title=checking?'Checking conversation history…':record.error||ready.available===false?'History check couldn’t finish':ready.scanComplete===false?'Couldn’t confirm enough history':'Autopilot needs more history';
 return `<div id="reply-modes">${modes}<div id="reply-readiness" class="reply-readiness" role="status" aria-busy="${checking}"><strong>${title}</strong>${checked&&ready.available!==false?`<p>${ready.countsAreMinimum?'At least ':''}${Number(ready.total)||0} usable messages · ${Number(ready.owner)||0} from you · ${Number(ready.incoming)||0} from them</p>`:''}${record.error||ready.available===false?`<p class="chat-log-error">${esc(record.error||ready.message||'History is temporarily unavailable. Try again.')}</p>`:''}<small>Needs 20 distinct messages, including 5 from each person. You can also add a chat log below.</small><button class="text-button" data-action="recheck-history" ${checking?'disabled':''}>${checking?'Checking…':'Recheck history'}</button></div></div>`;
}
...
async function saveRelationship(clear=false){
 ...
 if(edit.autoSend&&!profileEligibility(edit).eligible){toast('Recheck history or add a chat log before enabling Autopilot.');return;}
...
function historyLearningState(){const value=state.historyLearning||{};return {phase:'waiting',ready:false,processedMessages:0,totalMessages:0,completedContacts:0,totalContacts:0,error:'',...value};}
function historyLearningReady(){return historyLearningState().ready===true;}
function historyLearningActive(){return ['preparing','analyzing'].includes(historyLearningState().phase);}
function invalidateHistoryLearning(){historyLearningRequest=null;historyLearningRevision++;historyLearningNextCheck=0;state.historyLearning={phase:'waiting',ready:false};}
function historyLearningText(){const info=historyLearningState();if(info.ready)return 'Your message history is ready.';if(info.phase==='error')return info.error||'History preparation couldn’t finish. Try again.';if(!state.cloud?.configured)return 'Connect OpenAI to prepare your message history.';if(info.phase==='preparing')return 'Gathering message history from your phone…';if(info.phase==='analyzing')return 'Learning how you text each person…';return 'Message history needs to be prepared before AI replies are available.';}
function historyLearningProgress(){const info=historyLearningState(),number=value=>Math.max(0,Number(value)||0);return info.phase==='preparing'||info.phase==='analyzing'?`${number(info.processedMessages).toLocaleString()}${number(info.totalMessages)?' / '+number(info.totalMessages).toLocaleString():''} messages · ${number(info.completedContacts)}${number(info.totalContacts)?' / '+number(info.totalContacts):''} conversations`:'';}
function historyLearningView(){const info=historyLearningState(),active=historyLearningActive();return `<section class="panel history-learning-panel" id="history-learning-panel"><h3>Learn from message history</h3><p>Pilot reads all available SMS and MMS text history and sends it to your connected service and OpenAI to learn how you text each person. Attachments and unavailable RCS history aren’t included.</p><div class="history-learning-status" role="status" aria-busy="${active}">${active?'<span class="spinner"></span>':icon(info.ready?'check':'chat')}<div><strong>${esc(historyLearningText())}</strong>${historyLearningProgress()?`<small>${esc(historyLearningProgress())}</small>`:''}</div></div>${!info.ready&&!active?`<button class="secondary" data-action="retry-history-learning" ${!native||!state.cloud?.configured||!!historyLearningRequest?'disabled':''}>${info.phase==='error'?'Retry preparation':'Prepare history'}</button>`:''}<small>Preparation does not turn on Autopilot. You choose which people it can reply to.</small></section>`;}
function historyLearningNoticeView(){if(historyLearningReady()||!state.cloud?.configured)return '';return `<div class="history-learning-notice" id="history-learning-notice" role="status">${historyLearningActive()?'<span class="spinner"></span>':icon('spark')}<span>${esc(historyLearningText())} You can still write and send messages.</span><button class="text-button" data-action="history-learning-settings">Details</button></div>`;}
function syncHistoryLearningStatus(){const panel=document.getElementById('history-learning-panel');if(panel){const template=document.createElement('template');template.innerHTML=historyLearningView();if(panel.innerHTML!==template.content.firstElementChild.innerHTML)panel.replaceWith(template.content);}const composer=document.querySelector('.composer'),old=document.getElementById('history-learning-notice');if(composer){const template=document.createElement('template');template.innerHTML=historyLearningNoticeView();const fresh=template.content.firstElementChild;if(old&&fresh){if(old.innerHTML!==fresh.innerHTML)old.replaceWith(fresh);}else if(old)old.remove();else if(fresh)composer.prepend(fresh);}}
function checkHistoryLearning(retry=false){
 if(historyLearningRequest)return historyLearningRequest.promise;
 if(!native||!state.cloud?.configured||previewDenied||accessHint===false||document.visibilityState!=='visible'||!retry&&(!historyLearningActive()||Date.now()<historyLearningNextCheck))return Promise.resolve();
 const request={generation:privateGeneration,revision:historyLearningRevision,promise:null};historyLearningRequest=request;historyLearningNextCheck=Date.now()+1500;
 request.promise=(async()=>{try{const result=await api(retry?'retryHistoryLearning':'historyLearningStatus');if(historyLearningRequest!==request||request.generation!==privateGeneration||request.revision!==historyLearningRevision||previewDenied||accessHint===false||!state.cloud?.configured)return;
  if(!result||!['waiting','preparing','analyzing','ready','error'].includes(result.phase)||typeof result.ready!=='boolean')throw new Error('History preparation status is unavailable. Try again.');
  const wasReady=historyLearningReady();state.historyLearning=result;historyLearningRevision++;syncHistoryLearningStatus();if(wasReady!==historyLearningReady()){syncLiveComposer();document.querySelectorAll('[data-media-insight]').forEach(node=>syncMediaInsight(Number(node.dataset.mediaInsight)));if(historyLearningReady())queueMicrotask(maybeSuggestReply);}
 }catch(error){if(historyLearningRequest===request&&request.generation===privateGeneration&&request.revision===historyLearningRevision&&!previewDenied){state.historyLearning={...historyLearningState(),phase:'error',ready:false,error:error.message||'History preparation could not be checked.'};historyLearningRevision++;syncHistoryLearningStatus();syncLiveComposer();}}
 finally{if(historyLearningRequest===request){historyLearningRequest=null;syncHistoryLearningStatus();}}})();return request.promise;
}
...
async function refresh(force=false){
 ...
 refreshing=(async()=>{try{
  const setupRevision=roleStatusRevision,accessVersion=privateGeneration,learningRead=historyLearningRevision,snapshotRead=++snapshotSequence,inboxRead=++inboxReadSequence,s=await api('snapshot');if(accessVersion!==privateGeneration)return;if(s&&learningRead!==historyLearningRevision)s.historyLearning=state.historyLearning;if(!s||!Array.isArray(s.inbox))throw new Error('Your conversations could not be updated. Try again.');if(setupRevision!==roleStatusRevision){s.defaultSms=state.defaultSms;s.permissions=state.permissions;}const signature=JSON.stringify(s),previous=state,firstInbox=!inboxBootstrap.authoritative,newerInbox=overlayLiveInbox(s,inboxRead);inboxFullRead=inboxRead;
  const oldJob=JSON.stringify(pendingReply());state={theme:themeSaved,lockScreenPreviews:true,inAppSuggestions:true,linkPreviews:true,...s};if(JSON.stringify(previous.historyLearning)!==JSON.stringify(state.historyLearning))historyLearningRevision++;overlayMutations(state,snapshotRead,newerInbox);state.inbox=orderInboxRows(state.inbox);overlaySendJobs(state,snapshotRead,previous);if(accessHint===false||state.permissions===false||state.defaultSms===false){if(!previewDenied)clearPrivateChats();state.inbox=[];state.jobs=[];}else if(state.permissions===true&&state.defaultSms===true)previewDenied=false;if(contactsDenied||state.contactsAllowed===false){clearContactPhotos();state.inbox=state.inbox.map(row=>({...row,name:row.address,photo:''}));if(contactsDenied)state.contactsAllowed=false;}acceptFirstInbox();applyLaunchHistories();if(previous.contactPhotoRevision!==state.contactPhotoRevision)onContactPhotosChanged();if((previous.linkPreviews!==false)!==(state.linkPreviews!==false)){clearLinkPreviews();if(page==='inbox'&&conversation)updateTimeline();else if(page==='media-thread')syncMediaBrowse();}const present=new Set(state.inbox.map(row=>row.thread_id));for(const thread of chatPreviews.keys())if(!present.has(thread)){chatPreviews.delete(thread);if(thread!==current?.thread_id)threadHistories.delete(thread);}if(JSON.stringify(previous.inbox)!==JSON.stringify(state.inbox))invalidatePreviews(firstInbox);queuePreviewWarm();syncThemeUI();syncPinFeedback();syncPhoneSetup();syncContactSettings();syncLocationUI();syncHistoryCacheUI();syncHistoryLearningStatus();void checkHistoryLearning();
  ...
  if(changed||force){
   if(page==='inbox'){
    ...
    if(!busy&&current&&conversation&&(oldJob!==JSON.stringify(pendingReply())||previous.autoDraft!==state.autoDraft||previous.historyLearning?.ready!==state.historyLearning?.ready||JSON.stringify(previous.sleep)!==JSON.stringify(state.sleep)||force))syncLiveComposer();
  ...
  if((oldHistory!==historySignature(c.history)||c.readOnly!==previousConversation.readOnly||c.relationship?.cloudEnabled!==previousConversation.relationship?.cloudEnabled)&&page==='inbox')updateTimeline();syncHistoryLearning();syncApprovedLearning();fillHistoryGaps();rememberChat(thread,conversation,true);queueMicrotask(maybeSuggestReply);
...
async function maybeSuggestReply(){
 ...
 finally{if(suggestionRequest===request)suggestionRequest=null;syncLiveComposer();syncHistoryLearning();}
}
...
async function analyzeMedia(mediaId,automatic=false){
 if(!historyLearningReady()){if(!automatic)toast(historyLearningText());return;}
...
async function generate(unused=false,automatic=false){
 ...
 finally{if(draftRequest===request){draftRequest=null;busy=false;busyThread=null;syncLiveComposer();syncHistoryLearning();fillHistoryGaps();void refresh();}}
}

function tick(){void checkSendState();void checkHistoryLearning();document.querySelectorAll('[data-due]').forEach(el=>{const seconds=Math.max(0,Math.ceil((Number(el.dataset.due)-Date.now())/1000));el.textContent=seconds?`Sends in ${Math.floor(seconds/60)}:${String(seconds%60).padStart(2,'0')}`:'Timer finished · awaiting send status';});}
root.addEventListener('input',e=>{if(e.target.id==='home-address'){homeForm().address=e.target.value;homeForm().dirty=true;closeHomeCandidates(false);syncLocationUI();return;}if(e.target.id==='location-enabled'){changeLocation('saveLocation',{enabled:e.target.checked});return;}if(e.target.id==='pairing-code'){pairingText=e.target.value;root.querySelector('[data-action="connect-cloud"]').disabled=busy||!native||!pairingText.trim();}const testKey={'test-message':'message','test-relationship':'relationship','test-examples':'examples'}[e.target.id];if(testKey){cloudTest[testKey]=e.target.value;cloudTest.error='';document.getElementById('test-error').textContent='';root.querySelector('[data-action="test-run"]').disabled=!canTest();}if(e.target.name==='person-mode'){
 ...
 if(['auto-draft','match-style','in-app-suggestions','link-previews','lock-screen-previews','sim'].includes(e.target.id))preferences=readPreferences();if(e.target.id==='link-previews')clearLinkPreviews();
