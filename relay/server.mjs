...
const relay=createRelay({apiKey:secret,token,model:process.env.OPENAI_MODEL||'gpt-6-sol',maxDaily:200});
async function handler(req,res){
 ...
 try{
  ...
  if(!['/health','/draft','/media-analysis','/train','/history-analysis'].includes(req.url))throw new PublicError(404,'Not found.');
  if(req.method==='POST'&&!String(req.headers['content-type']).startsWith('application/json'))throw new PublicError(415,'Use JSON.');
  const limit=req.url==='/media-analysis'?1048576:262144;
